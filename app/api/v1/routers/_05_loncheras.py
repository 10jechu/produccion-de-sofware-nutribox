...
def crear_lonchera(payload: LoncheraCreate, db: Session = Depends(get_session)):
    try:
...
def listar_loncheras(db: Session = Depends(get_session)):
    return db.query(Lonchera).all()

@router.get("/{id}", response_model=LoncheraRead)
def obtener_lonchera(id: int, db: Session = Depends(get_session)):
...
def actualizar_lonchera(id: int, payload: LoncheraUpdate, db: Session = Depends(get_session)):
    obj = db.get(Lonchera, id)
    if not obj:
        raise HTTPException(status_code=404, detail="Lonchera no encontrada")
...
def eliminar_lonchera(id: int, db: Session = Depends(get_session)):
    obj = db.get(Lonchera, id)
    if not obj:
        raise HTTPException(status_code=404, detail="Lonchera no encontrada")
    db.delete(obj)
